...
前端界面正中心为一个3×5老虎机（找一个前端控件实现以下）不需要做动画，能够更新结果就行，有一个按钮触发老虎机，有计数器显示轮数，剩余回合数，持有量子数，圣晶石数量，有一个物品栏显示持有从者，可以点击打开背包，更换上场从者，触发事件时有选择框选择事件。
